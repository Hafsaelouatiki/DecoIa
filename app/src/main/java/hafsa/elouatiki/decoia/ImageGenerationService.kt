        ...
        try {
            val payload = JSONObject().apply {
                ...
                if (originalBitmap != null) {
                    ...
                    bytes.forEach { imageArray.put(it.toInt() and 0xFF) }
                    put("image", imageArray)
                }
            }

            Log.e("ImageGenerationService", "URL = $url")
            Log.e("ImageGenerationService", "Prompt = $prompt")
            ...
            client.newCall(request).execute().use { response ->
                ...
                Log.e("ImageGenerationService", "HTTP CODE = ${response.code}")
                Log.e("ImageGenerationService", "CONTENT TYPE = $contentType")
                Log.e("ImageGenerationService", "BYTES SIZE = ${responseBytes?.size ?: 0}")

                if (responseBytes == null || responseBytes.isEmpty()) {
                    Log.e("ImageGenerationService", "Réponse vide")
                    return@withContext null
                }

                if (!response.isSuccessful) {
                    val errorText = responseBytes.toString(Charsets.UTF_8)
                    Log.e("ImageGenerationService", "ERROR BODY = $errorText")
                    return@withContext null
                }
                ...
                Log.e("ImageGenerationService", "Image sauvegardée = ${file.absolutePath}")

                return@withContext file.absolutePath
            }

        } catch (e: Exception) {
            Log.e("ImageGenerationService", "Erreur génération image", e)
            null
        }
    }
